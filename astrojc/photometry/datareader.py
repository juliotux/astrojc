...
def read_wise(fname, mjd, mjd_limit=10, deviation_limit=0.2):
    ...
    '''
    # TODO: follow the read_allwise example and reimplement it using
    # _read_wise_band
...
def read_allwise(table, fname, mjd, mjd_limit=10, deviation_limit=0.2):
    ...
    med_w4 = np.nanmedian(w4)
    for W1, W2, W3, W4, Sig1, Sig2, Sig3, Sig4 in zip(w1, w2, w3, w4,
                                                      w1_err, w2_err,
                                                      w3_err, w4_err):
...
class LTPV_data():
    ...
    def get_id(self):
        ...
        for i in range(1, 5):
            mycat = ascii.read(self.data_dir+'stars'+str(i),
                               readme=self.data_dir+'ReadMe'+str(i))
            for j in mycat:
                if str(j['HD']).replace(' ', '') == star_name or \
                   str(j['HR']).replace(' ', '') == star_name or \
                   str(j['DM']).replace(' ', '') == star_name or \
                   str(j['Name2']).replace(' ', '') == star_name:
                    star = j['Name']
        if star == '':
    ...
    def load_data(self):
        tempdata = []
        for i in range(1, 5):
            mycat = ascii.read(self.data_dir+'ltpv'+str(i),
                               readme=self.data_dir+'ReadMe'+str(i))
...
def read_generic_table(table, jd_key, mag_key, mag_err_key=None, jd_correct=0):
    '''
    Read a generic astropy table containing the data and correct the Julian
    Date by a value if it's needed (like the data in MJD).
    '''
    t = Table()
