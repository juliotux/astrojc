...
def plot_lightcurve(ax, jd, flux, jd_error=None, flux_error=None, offset=0,
                    fmt='ko', label=None, **pltkwargs):
    if flux_error is not None or jd_error is not None:
        ax.errorbar(jd, flux + offset, xerr=jd_error, yerr=flux_error,
                    fmt=fmt, label=label, **pltkwargs)
    else:
